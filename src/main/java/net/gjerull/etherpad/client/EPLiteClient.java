...
public class EPLiteClient {
	...
	/**
	 * Initializes a new net.gjerull.etherpad.client.EPLiteClient ç object.
	 * The default Etherpad Lite API version (in DEFAULT_API_VERSION) will
	 * be used.
	 *
	 ...
	 *
	 * @param url        an absolute url, including protocol, to the EPL
	 *                   api
	 * @param apiKey     the API Key
	...
	/**
	 * Creates a new Group. The group id is returned in "groupID" in the
	 * Map.
	 *
	...
	/**
	 * List all the padIDs in a group. They will be in an array inside
	 * "padIDs".
	 *
	 * @param groupID string
	 * @return Map
	 ...
	 */
	public final Map createGroupPad(final String groupID,
			final String padName, final String text) {
		Map<String, Object> args = new HashMap<>();
		args.put("groupID", groupID);
		args.put("padName", padName);
		args.put("text", text);
	...
	/**
	 * Create a new author with the given name. The author id is returned
	 * in "authorID".
	 *
	 * @param name string
	 * @return Map with authorID
	 ...
	 * @return Map with authorID
	 */
	public final Map createAuthorIfNotExistsFor(
			final String authorMapper) {
		Map<String, Object> args = new HashMap<>();
	...
	// with
	// a valid session for this group, can access group pads. You can
	// create a
	...
	/**
	 * Create a new session for the given author in the given group, valid
	 * until the given UNIX time. <br />
	 * Example:<br />
	 * <br />
	 * <code>
	...
	/**
	 * Create a new session for the given author in the given group valid
	 * for the given number of hours. <br />
	 * Example:<br />
	 * <br />
	 * <code>
	 * // in 2 hours<br />
	 ...
	 */
	public final Map createSession(final String groupID,
			final String authorID, final int sessionDuration) {
		long inNHours = ((new Date()).getTime()
				+ ((long) sessionDuration * 60L * 60L * 1000L)) / 1000L;
		return this.createSession(groupID, authorID, inNHours);
	}

	/**
	 * Create a new session for the given author in the given group, valid
	 * until the given datetime. <br />
	 * Example:<br />
	 * <br />
	 * <code>
	 * import java.util.Date;<br />
	...
	/**
	 * Returns information about a session: authorID, groupID and
	 * validUntil.
	 *
	 * @param sessionID string
	 * @return Map
	...
	/**
	 * List all the sessions IDs in a group. Returned as a Map of
	 * sessionIDs keys, with values of Maps containing groupID, authorID,
	 * and validUntil.
	 *
	 * @param groupID string
	 * @return Map
	...
	/**
	 * List all the sessions IDs belonging to an author. Returned as a Map
	 * of sessionIDs keys, with values of Maps containing groupID,
	 * authorID, and validUntil.
	 *
	 * @param authorID string
	 * @return Map
	 */
	...
	/**
	 * Returns a Map containing the latest revision of the pad's text. The
	...
	/**
	 * Returns a Map containing the current revision of the pad's text as
	...
	/**
	 * Returns a Map containing the a specific revision of the pad's text
	 * as HTML. The html is stored under "html".
	 *
	 * @param padId the pad's id string
	 * @param rev   the revision number
	...
	public final String getRevisionChangeset(final String padId) {
		Map<String, Object> args = new HashMap<>();
		args.put("padID", padId);
		return (String) this.connection.getObject("getRevisionChangeset",
				args);
	}
	 ...
	 */
	public final Map createDiffHTML(final String padId,
			final long startRev, final long endRev) {
		Map<String, Object> args = new HashMap<>();
		args.put("padID", padId);
	...
	/**
	 * Returns the chatHead (last number of the last chat-message) of the.
	 * pad
	 *
	 * API >= 1.2.7
	 *
	...
	public final Map appendChatMessage(final String padId,
			final String text, final String authorId) {
		Map<String, Object> args = new HashMap<>();
		args.put("padID", padId);
		args.put("text", text);
		args.put("authorID", authorId);
	 ...
	 */
	public final Map appendChatMessage(final String padId,
			final String text, final String authorId, final long time) {
		Map<String, Object> args = new HashMap<>();
		args.put("padID", padId);
		args.put("text", text);
		args.put("authorID", authorId);
	...
	/**
	 * Returns the list of users that are currently editing this pad. A
	 * padUser has the values: "colorId", "name" and "timestamp".
	 *
	 * @param padId the pad's id string
	...
	/**
	 * Copies a pad with full history and chat. If the destination exists
	 * the copy will fail.
	 *
	 * API >= 1.2.8
	 *
	 ...
	 * @param sourcePadId      the id of the source pad
	 * @param destinationPadId the id of the destination pad
	 * @param force            if force is true and the destination pad
	 ...
	 *
	 * @param padId        the pad's id string
	 * @param publicStatus final boolean
	 */
