...
/**
 * Connection object for talking to and parsing responses from the Etherpad
 * Lite Server.
 */
public class EPLiteConnection {
	...
	/**
	 * Initializes a new net.gjerull.etherpad.client.EPLiteConnection
	 * object.
	 *
	...
	public final Map get(final String apiMethod) {
		final Map response = (Map) this.getObject(apiMethod);
	 ...
	 *
	 * @param apiMethod the name of the API method to call
	 * @param apiArgs   a HashMap of url/form parameters. apikey will be
	 *                  set automatically
	 * @return Object
	 */
	public final Object getObject(final String apiMethod,
			final Map<String, Object> apiArgs) {
		final String path = this.apiPath(apiMethod);
		final String query = this.queryString(apiArgs, false);
		final URL url = apiUrl(path, query);
	...
	public final Map get(final String apiMethod,
			final Map<String, Object> apiArgs) {
		final Map response = (Map) this.getObject(apiMethod, apiArgs);
		return (response != null) ? response : new HashMap();
	}
	...
	public final Map post(final String apiMethod) {
		final Map response = (Map) this.postObject(apiMethod);
		return (response != null) ? response : new HashMap();
	}
	...
	public final Object postObject(final String apiMethod,
			final Map<String, Object> apiArgs) {
		final String path = this.apiPath(apiMethod);
		final String query = this.queryString(apiArgs, true);
		final URL url = apiUrl(path, null);
		final Request request = new POSTRequest(url, query);
		return this.call(request);
	}
	...
	public final Map post(final String apiMethod,
			final Map<String, Object> apiArgs) {
		final Map response = (Map) this.postObject(apiMethod, apiArgs);
		return (response != null) ? response : new HashMap();
	}
	 ...
	 * @return Object
	 */
	protected Object handleResponse(final String jsonString) {
		try {
			final JSONParser parser = new JSONParser();
			final Map response = (Map) parser.parse(jsonString);
			// Act on the response code
			if (response.get("code") != null) {
				final int code = ((Long) response.get("code")).intValue();
				switch (code) {
				...
				default:
					throw new EPLiteException(
							"An unknown error has occurred while handling "
							+ "the response: "
									+ jsonString);
				}
			...
			throw new EPLiteException(
					"Unable to parse JSON response (" + jsonString + ")",
					e);
		}
	}
	 ...
	 * @return URL
	 */
	protected URL apiUrl(final String path, final String query) {
		try {
			return new URL(
					new URI(this.uri.getScheme(), null, this.uri.getHost(),
							this.uri.getPort(), path, query, null)
									.toString());
	 ...
	 * @return String
	 */
	protected String apiPath(final String apiMethod) {
		return this.uri.getPath() + "/api/" + this.apiVersion + "/"
				+ apiMethod;
	}
	 ...
	 * @return String
	 */
	protected String queryString(final Map<String, Object> apiArgs,
			final boolean urlEncode) {
		final StringBuilder strArgs = new StringBuilder();
		apiArgs.put("apikey", this.apiKey);
		final Iterator i = apiArgs.entrySet().iterator();
		while (i.hasNext()) {
